...
from src.openai_backend.openai_audio_backend import OpenAIAudioConfigManager


class TestConfigManager(unittest.TestCase):
    def setUp(self):
        """Set up the test case with a derived ConfigManager."""
        self.config_manager = OpenAIAudioConfigManager(
            transcription={"model": "whisper-2", "speed": "2x"}, text_to_speech={"voice": "voice3"}
        )
    ...
    def test_combine_config(self):
        ...
        combined_config = self.config_manager.combine_config(
            "transcription", model="whisper-3", speed="1.5x", new_param="new_value"
        )
